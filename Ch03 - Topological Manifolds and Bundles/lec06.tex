...
\begin{definition}[Topological Manifold]
	A paracompact Hausdorff topological space \((M, \CO)\) is called a \emph{\(d\)-dimensional (topological) manifold} if for every \(p \in M\) there exists an open neighborhood \(U_p \in \CO\) of \(p\) and a homeomorphism \(x: U_p \to x(U_p) \subseteq \R^d\). And we write \(\dim{M} = d\).
\end{definition}
...
\begin{definition}[Submanifold]
	Let \((M, \CO)\) be a topological manifold and \(N \subseteq M\) be a subset. Then \((N, \eval{\CO}_N)\) is called a \emph{submanifold} of \((M, \CO)\) if it is a topological manifold in its own right.
\end{definition}
...
\begin{definition}[Chart]\label{def:chart}
	Let \((M, \CO)\) be a \(d\)-dimensional manifold. Then the pair \((U, x)\), where \(U \in \CO\) and \(x: U \to x(U) \subseteq \R^d\) is a homeomorphism, is called a \emph{chart} on \(M\).
\end{definition}
...
\begin{definition}[Atlas]\label{def:atlas}
	Let \((M, \CO)\) be a \(d\)-dimensional manifold. Then the collection of all charts \(\A := \qty{(U_\alpha, x_\alpha) \mid \alpha \in I}\) is called an \emph{atlas} of the manifold \(M\) if,
	\begin{equation}
...
\begin{example}
	Let \((\R, \COstd)\) be our topological manifold. Then we have a trivial atlas \(\A = \qty{(\R, \id_{\R})}\), but it is not maximal. Since chart \(((0, 1), \id_{(0, 1)})\) is \(\SC^0\)-compatible with the chart \((\R, \id_{\R})\), but it is not in the atlas \(\A\).
\end{example}
