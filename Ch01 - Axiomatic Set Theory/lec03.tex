...
\begin{itemize}
	...
	\item The set \(\phi(A) \equiv \im_\phi(A) := \qty{\phi(a) \in B \mid a \in A}\) is called the \emph{image} of \(A\) under \(\phi\).
\end{itemize}

\begin{definition}
	Let \(A\) and \(B\) be sets. A map \(\phi: A \to B\) is called
	\begin{itemize}
		\item \emph{Surjective} (or \emph{onto}) if and only if \(\im_\phi(A) = B\).
