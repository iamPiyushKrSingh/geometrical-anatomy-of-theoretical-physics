...
\begin{definition}[T1 Space]\label{def:t1_space}
	A topological space \((M, \CO)\) is called \emph{T1} if for any two distinct points \(p, q \in M\) (\ie\ \(p \neq q\)), then
	\begin{equation}
...
\begin{definition}[T2 Space]\label{def:t2_space}
	A topological space \((M, \CO)\) is called \emph{T2} or \emph{Hausdorff} if for any two distinct points \(p, q \in M\) (\ie\ \(p \neq q\)), then
	\begin{equation}
		\exists U_p, V_q \in \CO: U_p \cap V_q = \0 \label{eq:separation_axiom_t2}
	\end{equation}
...
\begin{remark}[T$2\frac{1}{2}$ Space]
	A topological space \((M, \CO)\) is called \emph{T$2\frac{1}{2}$} if for any two distinct points \(p, q \in M\) (\ie\ \(p \neq q\)), then there exists a closed neighbourhood \(U_p\) of \(p\) such that \(q \notin U_p\).
\end{remark}

\begin{theorem}[Uniqueness of Limit Point]\label{thm:uniqueness_limit_point}
	Let \((M, \CO)\) be a Hausdorff space. Then every convergent sequence in \(M\) has a unique limit point.
\end{theorem}
...
\begin{definition}[Cover and Open Cover]\label{def:cover}
	Let \((M, \CO)\) be a topological space. A set \(C \subseteq \powerset(M)\) is called a \emph{cover} of \(M\) if
	\begin{equation}
		\bigcup C = M. \label{eq:cover}
	\end{equation}
	If \(C \subseteq \CO\) then it is called an \emph{open cover}.
\end{definition}
...
\begin{definition}[Compact Space]\label{def:compact_space}
	A topological space \((M, \CO)\) is called \emph{compact} if every open cover of \(M\) has a finite subcover.
\end{definition}
...
\begin{definition}[Compact Subset]\label{def:compact_subset}
	Let \((M, \CO)\) be a topological space. A subset \(N \subseteq M\) is called \emph{compact} if the induced topology \((N, \eval{\CO}_N)\) is compact.
\end{definition}
...
\begin{theorem}[Heine-Borel Theorem]
	Let \((\R^d, \COstd)\) be a topological space. Then a subset \(K \subseteq \R^d\) is compact if and only if it is closed and bounded.
\end{theorem}
...
We can extend the Heine-Borel Theorem to any metric space. But first we need to define the concept of `metric space'.
...
\begin{example}
	...
	\begin{enumerate}[(a)]
		\item \([0, 1]\) is compact in \((\R, \COstd)\).

		\item \(\R\) is not compact in \((\R, \COstd)\). It suffices to show that there exists an open cover of \(\R\) which does not have a finite subcover. Consider the open cover
		      \begin{equation}
...
\begin{definition}[Refinement of a Cover]\label{def:refinement_cover}
	...
	\begin{itemize}
		\item \emph{open} if \(R \subseteq \CO\).
...
\begin{definition}[Paracompact Space]\label{def:paracompact_space}
	A topological space \((M, \CO)\) is called \emph{paracompact} if every open cover of \(M\) has a locally finite open refinement.
\end{definition}
...
\begin{definition}[Metrisable Space]
	A topological space \((M, \CO)\) is called \emph{metrizable} if there exists a metric \(d\) on \(M\) such that the metric-induced topology \(\O_d\) is same as \(\CO\).
\end{definition}
...
\begin{example}
	...
	\begin{enumerate}[(a)]
		\item The topological space \((\R^d, \COstd)\) is metrizable since \(\COstd = \O_d\) where \(d = \norm{\cdot}_2\). Hence, \((\R^d, \COstd)\) is paracompact.
...
\begin{theorem}
	Let \((M, \CO)\) be a paracompact space and \((N, \O_N)\) be a compact space. Then the product space \((M \times N, \O_{M \times N})\) is paracompact.
\end{theorem}

\begin{corollary}
	Let \((M, \CO)\) be a paracompact space and \((N_i, \O_{N_i})\) be compact spaces for all \(i \in \qty{1, 2, \ldots, n}\). Then the product space \(M \times N_1 \times N_2 \times \ldots \times N_n\) is paracompact.
\end{corollary}
...
\begin{definition}[Partition of Unity]\label{def:partition_of_unity}
	Let \((M, \CO)\) be a topological space. A set \(\mathcal{F}\) of continuous functions \(\phi: M \to [0, 1]\) is called a \emph{partition of unity} if for each \(p \in M\),
	\begin{enumerate}[(a)]
...
\begin{definition}[Partition of Unity Subordinate to a Cover]\label{def:partition_of_unity_subordinate}
	Let \((M, \CO)\) be a topological space and let \(C\) be a cover of \(M\). A partition of unity \(\mathcal{F}\) is said to be \emph{subordinate} to \(C\) if:
	\begin{equation}
...
\begin{theorem}
	Let \((M, \CO)\) be a Hausdorff space. Then \(M\) is paracompact if and only if every open cover \(C\) admits a partition of unity subordinate to \(C\).
\end{theorem}

\begin{example}
	Let \((\R, \COstd)\) be a topological space. By \cref{thm:stone}, \((\R, \COstd)\) is paracompact. Consider the open cover \(C := \qty{U_1 = (-\infty, 1), U_2 = (0, \infty)}\). We can construct a partition of unity \(\mathcal{F} = \qty{\phi_1, \phi_2}\) subordinate to \(C\) as follows:
	\begin{align*}
		\phi_1(x) = \begin{cases}
			            1       & \text{if}\ x \le 0       \\
...
\begin{definition}[Connected Space]\label{def:connected_space}
	A topological space \((M, \CO)\) is called \emph{connected} unless there exists two non-empty open sets \(A, B\) such that
	\begin{equation}
...
\begin{example}
	Let \(\qty(M := \R \setminus \qty{0}, \CO := \eval{\COstd}_{\R \setminus \qty{0}})\) be a topological space. Then \(M\) is not connected since we can write \(M = A \cup B\) where \(A = (-\infty, 0)\) and \(B = (0, \infty)\) with \(A \cap B = \0\).
\end{example}

\begin{theorem}
	The closed interval \([0, 1]\) with induced topology from \((\R, \COstd)\) is connected.
\end{theorem}

\begin{theorem}
	Let \((M, \CO)\) be a topological space. Then \(M\) is connected if and only if \(\0\) and \(M\) are the only subsets of \(M\) that are both open and closed.
\end{theorem}
...
\begin{definition}[Path-Connected Space]\label{def:path_connected_space}
	A topological space \((M, \CO)\) is called \emph{path-connected} if for every pair of points \(p, q \in M\), there exists a continuous curve \(\gamma: [0, 1] \to M\) such that \(\gamma(0) = p\) and \(\gamma(1) = q\).
\end{definition}

\begin{example}
	Let \((\R^d, \COstd)\) be a topological space. Then \((\R^d, \COstd)\) is path-connected.
\end{example}
\begin{proof}
...
\begin{example}
	Let \(S := \qty{\qty(x, \sin(\frac{1}{x})) \mid x \in (0, 1]} \cup \qty{(0, 0)} \subseteq \R^2\). Then \((S, \eval{\COstd}_{S})\) is not path-connected but connected.
	\begin{figure}[H]
		\centering
...
\begin{definition}[Homotopic Curves]\label{def:homotopic_curves}
	Let \((M, \CO)\) be a topological space. Two continuous curves \(\gamma_1, \gamma_2: [0, 1] \to M\) such that
	\begin{equation*}
...
\begin{proposition}[Homotopy is an Equivalence Relation]\label{prop:homotopy_equivalence_relation}
	Let \((M, \CO)\) be a topological space. Define a relation \(\sim\) on the set of continuous curves \(\qty{\gamma: [0, 1] \to M}\) by
	\begin{equation}
...
\begin{definition}[Space of Loops]\label{def:space_of_loops}
	Let \((M, \CO)\) be a topological space. Then, for every point \(p \in M\), the set \(\mathscr{L}_p\) defined as
	\begin{equation}
...
\begin{definition}[Concatenation of Loops]\label{def:concatenation_of_loops}
	Let \((M, \CO)\) be a topological space. Fix a point \(p \in M\). We define the \emph{concatenation} operation \(\ast_p: \mathscr{L}_p \times \mathscr{L}_p \to \mathscr{L}_p\) as
	\begin{equation}
...
\begin{definition}[Fundamental Group]\label{def:fundamental_group}
	Let \((M, \CO)\) be a topological space and let \(p \in M\). Define the set \(\pi_1(p)\) as
	\begin{equation}
...
\begin{example}[2-sphere \(S^2\)]
	...
	\end{equation}
	We define the topological space \((S^2, \CO)\) where \(\CO\) is the induced topology from \((\R^3, \COstd)\).
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}
			% Draw a sphere
