...
\begin{definition}[Topology \& Topological Space]
	Let \(M\) be a set. Then a choice of subsets \(\CO \subseteq \powerset(M)\) is called a \emph{topology} on \(M\) if
	\begin{enumerate}[(i)]
		\item \(\0 \in \CO\) and \(M \in \CO\),
		\item \(U, V \in \CO \implies \bigcap \qty{U, V} \in \CO\),
		\item \(C \subseteq \CO \implies \bigcup C \in \CO\).
	\end{enumerate}
	The pair \((M, \CO)\) is then called a \emph{topological space}.
\end{definition}
\begin{remark}[Open \& Closed Sets]
	Let \((M, \CO)\) be a topological space. Then
	\begin{enumerate}[(a)]
		\item A set \(U \in \CO\) is called an \emph{open set} in \((M, \CO)\).
		\item A set \(C \subseteq M\) is called a \emph{closed set} in \((M, \CO)\) if \(M \setminus C \in \CO\) \ie\ \(M \setminus C\) is open in \((M, \CO)\).
	\end{enumerate}
\end{remark}

\begin{remark}[choice of topology]
	Unless \(\abs{M} = 1\), there are different topologies \(\CO\) one can choose on one and the same set \(M\). Based on the choice of topology \(\CO\), the notion of convergence of sequences and continuity of maps will change.
\end{remark}
...
\begin{example}[Basic Topologies]
	...
	\begin{enumerate}[(a)]
		\item \(\CO = \qty{\0, M}\). It is easy to see that, \(\CO\) is a topology on \(M\). This is called the \emph{chaotic topology}.

		\item \(\CO = \powerset(M)\). It is easy to see that, \(\CO\) is a topology on \(M\). This is called the \emph{discrete topology}.
	\end{enumerate}
\end{example}
For a less abstract example, consider \(M = \qty{1, 2, 3}\), then we can define 29 different topologies on \(M\). For example, \(\CO = \qty{\0, \qty{1}, \qty{2}, \qty{1, 2}, \qty{1, 2, 3}}\) is a topology on \(M\).

\begin{example}[Standard Topology on \(\R^d\)]
	This is the most important and heavily used topology in mathematics and physics.\\
	Consider \(M = \R^d\), where \(d \in \N\). Then the \emph{standard topology} on \(\COstd\) is constructed as follows:
	\begin{equation*}
		U \in \COstd :\iff \forall p \in U: \exists r \in \R^+: B_r(p) \subseteq U \label{eq:standard_topology}
	\end{equation*}
	...
	\begin{equation*}
		\COstd = \qty{U \in \powerset(\R^d) \mid \forall p \in U: \exists r \in \R^+: B_r(p) \subseteq U} \label{eq:standard_topology_explicit}
	\end{equation*}
	Then by \cref{thm:PRC}, \(\COstd\) is a set. Now we have to prove that \(\COstd\) is indeed a topology on \(\R^d\).
	\begin{enumerate}[(i)]
		\item First, we need to check whether \(\0 \in \COstd\), \ie\ whether:
		      \begin{equation*}
		      ...
		      \end{equation*}
		      However, since \(p \in \0\) is false, the implication is true independent of \(p\). Hence, the initial proposition is true and thus \(\0 \in \COstd\).
		      ...
		      \end{equation*}
		      is true and thus \(\R^d\in\COstd\).

		\item Let \(U, V \in \COstd\), then we have to show that \(U\cap V\in\COstd\). Let \(p \in U\cap V\), then \(p \in U\) and \(p \in V\). Since \(U, V \in \COstd\), we have:
		      \begin{align*}
			      \exists r_1 \in \R^+ : B_{r_1}(p) & \subseteq U, \\
			      \exists r_2 \in \R^+ : B_{r_2}(p) & \subseteq V.
		      \end{align*}
		      Now, let \(r = \min\qty{r_1, r_2}\), then we have \(B_r(p) \subseteq B_{r_1}(p) \subseteq U\) and \(B_r(p) \subseteq B_{r_2}(p) \subseteq V\). Hence, \(B_r(p) \subseteq U\cap V\) and thus \(U\cap V\in\COstd\).

		\item Let \(C \subseteq \COstd\), then we have to show that \(\bigcup C\in\COstd\). Let \(p \in \bigcup C\), then \(p \in U\) for some \(U \in C\). Since \(U \in \COstd\), we have:
		      \begin{equation*}
			      \exists r \in \R^+ : B_r(p) \subseteq U \subseteq \bigcup C.
		      \end{equation*}
		      Hence, \(\bigcup C \in \COstd\).
	\end{enumerate}
	Thus, \(\COstd\) is a topology on \(\R^d\).
\end{example}
...
\begin{theorem}[Induced Topology]\label{thm:induced_topology}
	Let \((M, \CO)\) be a topological space and \(N \subset M\)\footnotemark. Then the following set
	\begin{equation}
		\eval{\CO}_N = \qty{U \cap N \mid U \in \CO} \subseteq \powerset(N) \label{eq:induced_topology}
	\end{equation}
...
\begin{proof}
	We have to show that \(\eval{\CO}_N\) is a topology on \(N\).
	\begin{enumerate}[(i)]
		\item First, as \(\0 \in \CO\), we have \(\0 \cap N = \0 \in \eval{\CO}_N\).

		      Second, since \(M \in \CO\), we have \(M \cap N = N \in \eval{\CO}_N\).

		\item \(U, V \in \eval{\CO}_N \xRightarrow{?} U \cap V \in \eval{\CO}_N\). From \eqref{eq:induced_topology}, \(\exists U_1, V_1 \in \CO: U = U_1 \cap N \land V = V_1 \cap N\). Then we have:
		      \begin{equation*}
			      U \cap V = (U_1 \cap N) \cap (V_1 \cap N) = (U_1 \cap V_1) \cap N \in \eval{\CO}_N
		      \end{equation*}
		      as \(U_1 \cap V_1 \in \CO\).

		\item \(C \subseteq \eval{\CO}_N \xRightarrow{?} \bigcup C\in\eval{\CO}_N\). Since \(C \subseteq \eval{\CO}_N\), we have \(C = \qty{U_i \cap N \mid U_i \in \CO}\). Then we have:
		      \begin{equation*}
			      \bigcup C = \bigcup \qty{U_i \cap N \mid U_i \in \CO} = \qty{\bigcup U_i} \cap N \in \eval{\CO}_N.
		      \end{equation*}
	\end{enumerate}
	Thus, \(\eval{\CO}_N\) is a topology on \(N\).
\end{proof}

\begin{example}
	Consider \((\R, \COstd)\) and \(N = \qty[0, 1] := \qty{x \in \R \mid 0 \le x \le 1}\). Then from \cref{thm:induced_topology}, \((N, \eval{\COstd}_N)\) is a topological space.

	It is easy to see that \((0, 1] \notin \COstd\) so \((0, 1]\) is not open in \((\R, \COstd)\), but \((0, 1] = (0, 2) \cap N \in \eval{\COstd}_N\). Hence, \((0, 1]\) is an open set in \((N, \eval{\COstd}_N)\).
\end{example}
At this point we can talk about openness and closedness of sets in a topological space.
\begin{remark}
	Let \((M, \CO)\) be a topological space and \(N \subset M\). Then \(N\) is either
...
\begin{example}
	For any topological space \((M, \CO)\), we know by definition of topology that \(\0, M \in \CO\). Hence, \(\0\) and \(M\) are open in \((M, \CO)\).
	\begin{itemize}
		\item Now, let \(N = M\), then \(M \setminus N = \0 \in \CO\). Hence, \(M\) is closed in \((M, \CO)\).
		\item Let \(N = \0\), then \(M \setminus N = M \in \CO\). Hence, \(\0\) is closed in \((M, \CO)\).
	\end{itemize}
	Thus, \(\0\) and \(M\) are both open and closed in \((M, \CO)\).
\end{example}
\subsection{Quotient Topology}
\begin{definition}[Quotient Topology]\label{def:quotient_topology}
	Let \((M, \CO)\) be a topological space and \(\sim\) be an equivalence relation on \(M\). Then the quotient set \(\faktor{M}{\sim}\) can be equipped with the \emph{quotient topology} \(\COq{M}\) defined as follows:
	\begin{equation}
		\COq{M} = \qty{U \subseteq \faktor{M}{\sim} \mid \bigcup U = \bigcup_{[u] \in U} [u] \in \CO} \label{eq:quotient_topology}
	\end{equation}
\end{definition}
...
\begin{equation}
	\COq{M} = \qty{U \subseteq \faktor{M}{\sim} \mid \preimg_{\pi}(U) \in \CO} \label{eq:quotient_topology_map}
\end{equation}
...
\begin{example}[Topology on 1-sphere]
	...
	\begin{enumerate}[(a)]
		\item \uline{Induced Topology:} As \(S^1 \subset \R^2\), we can construct a topology \(\CO\) on \(S^1\) by \cref{thm:induced_topology} using \((\R^2, \COstd)\) topological space. This is the \emph{standard topology} on \(S^1\).
		      \begin{figure}[H]
			      \centering
		      ...
		      \end{figure} \noindent
		      From \cref{fig:1-sphere_induced_topology}, we can see that \(U_i\)'s are open in \(\R^2\) and \(U_i \cap S^1\) are open in \(S^1\). See, \(U_3 \in \COstd \implies \0 \in \CO\) and \(U_1 \in \COstd \implies S_1 \in \CO\). Also, the set \(S_2\) is not open in \(S^1\) as \(\forall U \in \powerset(\R): U \cap S^1 \neq S_2\). Hence, \(S_2 \notin \CO\).
...
\noindent Let \((M, \CO)\) be a topological space. Let \(a \in M\) be a point. Then we say that a set \(U \in \CO\) is an \emph{open neighbourhood} of \(a\) if \(a \in U\).
...
\begin{equation*}
	\COstd {\;}_{\R^d} = \O_{\underbrace{\R \times \R \times \cdots \times \R}_{d\ \text{times}}}
\end{equation*}
...
\begin{definition}[Convergence and Limit Point]
	...
	\begin{equation}
		\forall U \in \CO: a \in U \implies \exists N \in \N: \forall n > N: q(n) \in U \label{eq:convergence}
	\end{equation}
\end{definition}
...
\begin{example}[Justifying different topologies]
	...
	\begin{enumerate}[(a)]
		\item \uline{Chaotic Topology:} Consider the chaotic topology \(\CO = \qty{\0, M}\) on \(M\). Then any sequence in \(M\) converges against any point in \(M\). Let \(a \in M\) and \(q\) be a sequence in \(M\), then we have only two choice for \(U \in \CO\), \ie\ \(U = \0\) or \(U = M\). In case \(U = \0\), the definition of convergence is vacuously true. In case \(U = M\), the definition of convergence is true as \(q(n) \in M\) for all \(n \in \N\). Hence, (arbitrary) sequence \(q\) converges against (arbitrary) point \(a\) in \((M, \CO)\).

		      This weird behavior is the reason for the name ``chaotic topology''.

		\item \uline{Discrete Topology:} Consider the discrete topology \(\CO = \powerset(M)\) on \(M\). Then only almost constant sequences converge against a (unique) point in \(M\). Let \(a \in M\) and \(q\) be a sequence in \(M\), then we have \(U = \qty{a} \in \CO\). Then the definition of convergence is true if and only if \(q(n) = a\) for all \(n > N\). Hence, only almost constant sequences converge against a point in \((M, \CO)\).
	\end{enumerate}
\end{example}
...
\begin{theorem}[Convergence in standard topology on \(\R^d\)]
	Let \(q\) be a sequence in \(\R^d\) and \(a \in \R^d\). Then \(q\) converges against \(a\) in \((\R^d, \COstd)\) if and only if
	\begin{equation}
...
\begin{proof}
	Let \(q\) be a sequence in \(\R^d\) and \(a \in \R^d\). Then \(q\) converges against \(a\) in \((\R^d, \COstd)\) if and only if
	\begin{equation}
		\forall U \in \COstd: a \in U \implies \exists N \in \N: \forall n > N: q(n) \in U \label{eq:convergence_Rd_1}
	\end{equation}
	From the definition of standard topology on \(\R^d\), we have \(U \in \COstd \implies \exists r \in \R^+: B_r(a) \subseteq U\). Hence, \eqref{eq:convergence_Rd_1} is equivalent to
	\begin{equation}
...
\begin{example}[Convergence in \(\R\)]
	Let \(q(n) = 1 - \frac{1}{n + 1}\) be a sequence in \(\R\). Then \(q\) converges against \(1\) in \((\R, \COstd)\) as
	\begin{equation*}
