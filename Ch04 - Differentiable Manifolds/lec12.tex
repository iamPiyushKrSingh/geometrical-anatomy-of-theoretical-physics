...
\begin{example}
    ...
    \begin{enumerate}[a)]
        ...
        If \(M\) is orientable, then there exits a nowhere vanishing \emph{top-form} \((n = \dim M)\) \(\omega\) on \(M\), providing a volume.
...
\begin{definition}[Wedge Product]
    ...
    \end{align*}
\end{definition}
Simplest example of wedge product is take two 1-forms \(\omega\) and \(\eta\), then
\begin{equation*}
...
Recall that given a smooth map \(\phi: M \to N\) between two smooth manifolds, we induce pull-backs \(\phi^*_p: \ST^*_{\phi(p)} N \to \ST^*_p M\) on the cotangent spaces for each \(p \in M\). With this global pull-back map \(\phi^*: \Gamma(\ST^* N) \equiv \Omega^1(N) \to \Gamma(\ST^* M) \equiv \Omega^1(M)\) is defined pointwise.
