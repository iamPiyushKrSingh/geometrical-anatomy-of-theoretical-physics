...
\begin{definition}[\(\flower\)-Atlas]
	Let \((M, \CO)\) be a \(d\)-dimensional manifold. An atlas \(\A\) is called \(\flower\)-atlas, if any two charts \((U, x), (V, y) \in \A\) are \(\flower\)-compatible.
\end{definition}
...
\begin{definition}[\(\SC^{k}\)-manifold]
	A triple \((M, \CO, \A)\) is called a \(\SC^{k}\)-manifold where
	\begin{itemize}
		\item \((M, \CO)\) is a topological manifold.
		\item \(\A\) is a maximal \(\SC^{k}\)-atlas on \(M\).
	\end{itemize}
\end{definition}

\begin{definition}[Incompatible Atlases]
	Let two \(\flower\)-compactible atlases \(\A_1\) and \(\A_2\) on a topological manifold \((M, \CO)\) be called compatible if \(\A_1 \cup \A_2\) is a $\flower$-atlas on \(M\). Otherwise, they are called incompatible.
\end{definition}

\begin{remark}
	A given topological manifold \((M, \CO)\) can have different incompatible atlases.
\end{remark}
...
\begin{equation}
	(\R, \COstd, \A_{1, \text{max}}) \diffIso (\R, \COstd, \A_{2, \text{max}})
\end{equation}
where \(\A_{1, \text{max}}\) and \(\A_{2, \text{max}}\) are the maximal \(\SC^{\infty}\)-atlases on \(\R\) defined in the previous example.

In principle, we want to know, how many different differentiable structures are there on a given topological manifold \((M, \CO)\) -- up to diffeomorphism? The answer to this question is not known in general, but we know that it depends on the dimension of the manifold \(M\).
